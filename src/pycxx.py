...
from .create_module import add_func_a
# TODO : paths 
python_header = "/usr/include/python3.10"

pybind11_header = "/home/matrix/GSoc/ipyhpx/pybind11/include"
flags = "-std=c++17"
...
class py11:
    ...
    def __remove_old_file(self, base, suffix):

        old_file = base + suffix + '.so'
        if os.path.exists(old_file):
            os.remove(old_file)

    def __write_new_src(self, base, version, fname, fun, rettype, args, oargs, cargs):
        version += 1
        suffix = "_v"+str(version)
        base += suffix
        src = self._call_write_src(fun,rettype,args,oargs,cargs,suffix)

        with open(base+'.so.txt', "w") as fd:
    ...
    def __execute_compilation(self, base, fname,fun,suffix):
        ...
        try:
            ...
            add_func_a(fun.__name__+suffix, self.lib_path, self.module_name)
